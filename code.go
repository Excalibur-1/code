...
// IntCode Int parse code int to error.
func IntCode(i int) Code {
	return Code(i)
}

// Equal equal a and b by code int.
func Equal(a, b Coder) bool {
	if a == nil {
		a = OK
	}
	if b == nil {
...
func Cause(err error) Coder {
	if err == nil {
		return OK
	}
	if ec, ok := errors.Cause(err).(Coder); ok {
		return ec
	}
...
func add(e int) Code {
	if _, ok := _codes[e]; ok {
		panic(fmt.Sprintf("metacode code: %d already exist\n", e))
	}
	_codes[e] = struct{}{}
	return IntCode(e)
}
