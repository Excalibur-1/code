...
// NewMetadata New creates an Metadata from a given key-value map.
...
func Pairs(kv ...interface{}) Metadata {
	if len(kv)%2 == 1 {
		panic(fmt.Sprintf("metacode: Pairs got the odd number of input pairs for metacode: %d\n", len(kv)))
	}
	md := Metadata{}
	var key string
	for i, s := range kv {
...
// ToString get string value from metacode in context
