...
func TestCopy(t *testing.T) {
	...
	orig := Pairs(key, val)
	copyMetaData := orig.Copy()
	if !reflect.DeepEqual(orig, copyMetaData) {
		t.Errorf("copied value not equal to the original, got %v, want %v", copyMetaData, orig)
	}
	orig[key] = "foo"
	if v := copyMetaData[key]; v != val {
...
func TestWithContext(t *testing.T) {
	md := Metadata(map[string]interface{}{RemoteIP: "127.0.0.1", Color: "red", Mirror: true})
...
func TestBool(t *testing.T) {
	md := Metadata{RemoteIP: "127.0.0.1", Color: "red"}
